...
@REGISTRY.register('Combine')
def osis_combine_post(eRowType: Literal["Post"], strCombineName: str, strAddCombineName: str,
                      dFactor: float = 1.0):
    """定义荷载组合的内容（子类型 Post，与 Material 等多函数共用同一 Combine 注册名）。

    命令列顺序：Combine,Post,CombineName,AddCombineName,Factor

    Args:
        eRowType (str): 固定为 Post，不区分大小写
...
@REGISTRY.register('Combine')
def osis_combine_solve(eRowType: Literal["CombineSolve"] = "CombineSolve"):
    """计算组合和包络（子类型 CombineSolve）。

    命令列顺序：Combine,CombineSolve

    Args:
        eRowType (str): 固定为 CombineSolve，可缺省
...
def osis_combine_del(
    eRowType: Literal["CombineDel"],
    strPara: Literal["All", "General", "Concrete", "Steel", "Composite"] | str,
):
    """删除荷载组合（子类型 CombineDel）。

    命令列顺序：Combine,CombineDel,Para

    Args:
        eRowType (str): 固定为 CombineDel，不区分大小写
