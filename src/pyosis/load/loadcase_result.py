...
def osis_load_case_result(fileName:str, eType:Literal['LCEF','LCED','LCND','LCBF','LCTL','LCS']) -> tuple[bool, str, Any]:
    ...
    is_ok, error, result_txt_path = e.OSIS_LoadCaseResult(fileName, eType)
    if is_ok:
        data = read_ansi_file_to_json(result_txt_path)
        return is_ok, error, json.dumps(data, indent=2, ensure_ascii=False)
    else:
        return is_ok, error, None



def read_ansi_file_to_json(file_path)-> list[dict]:
